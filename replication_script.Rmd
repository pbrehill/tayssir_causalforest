...
```{r}
het_forest_results <- distilled_causal_tree(binary_forest, seed = seed, num_candidate_trees = 1000, replicates = 2000, maxdepth = 3)
```

```{r}
het_results <- het_forest_results$estimates  %>%
  mutate(est = round(est, 3), se = round(se, 3)) %>%
  arrange(as.numeric(node))

het_results
write_clip(het_results)
```

```{r}
rpart.plot(het_forest_results$model)
```
